						...
						<tbody>
							...
							<?php foreach (cok("SELECT * FROM randevu INNER JOIN musteri ON musteri.musteri_id=randevu.musteri") as $key => $deger): ?>
								<tr>
									...
									<td>
										<a href="randevuduzenle.php?id=<?php echo $deger['id'] ?>" class="btn btn-success btn-sm">Edit</a>
										<a href="randevuduzenle.php?id=<?php echo $deger['id'] ?>" class="btn btn-danger btn-sm">Delete</a>
									</td>
								</tr>
							<?php endforeach ?>
